# 📚 Complete Documentation Index

Welcome to the Markdown Slides documentation! This index helps you find exactly what you need.

## 🚀 Getting Started

Start here if you're new to the project:

| Document | Description | Time |
|----------|-------------|------|
| [README.md](./README.md) | Project overview, features, and quick start | 5 min |
| [QUICKSTART_CLI.md](./QUICKSTART_CLI.md) | Get started with CLI in 60 seconds | 2 min |
| [CLI_USAGE_GUIDE.md](./CLI_USAGE_GUIDE.md) | Complete CLI reference and troubleshooting | 10 min |
| [QUICKSTART_RELEASE.md](./QUICKSTART_RELEASE.md) | Create your first release in 5 minutes | 5 min |
| [USER_GUIDE.md](./USER_GUIDE.md) | Complete guide to using the application | 15 min |

## 🚢 Release & Automation

Everything about automated releases:

| Document | Description | Audience |
|----------|-------------|----------|
| [QUICKSTART_RELEASE.md](./QUICKSTART_RELEASE.md) | Quick start guide for releases | Everyone |
| [VERSION_BUMP_QUICKREF.md](./VERSION_BUMP_QUICKREF.md) | **⭐ Quick reference for version bumps and pre-releases** | Developers |
| [PRERELEASE_GUIDE.md](./PRERELEASE_GUIDE.md) | Complete alpha/beta/RC guide | Developers |
| [RELEASE_WORKFLOW.md](./RELEASE_WORKFLOW.md) | Comprehensive workflow documentation | Developers/DevOps |
| [WORKFLOW_ARCHITECTURE.md](./WORKFLOW_ARCHITECTURE.md) | Visual diagrams and architecture | Developers/DevOps |
| [RELEASE_IMPLEMENTATION.md](./RELEASE_IMPLEMENTATION.md) | Implementation details and summary | Technical leads |
| [.github/RELEASE_GUIDE.md](./.github/RELEASE_GUIDE.md) | Quick reference for creating releases | Developers |
| [.github/RELEASE_TEMPLATE.md](./.github/RELEASE_TEMPLATE.md) | Template for release notes | Developers |
| [scripts/README.md](./scripts/README.md) | Documentation for helper scripts | Developers |

## 📦 Deployment & Publishing

Deploy your application to various platforms:

| Document | Description | Platform |
|----------|-------------|----------|
| [QUICKSTART_AUTOMATED_RELEASE.md](./QUICKSTART_AUTOMATED_RELEASE.md) | **⭐ NEW: 5-min automated release setup** | All platforms |
| [AUTOMATED_NPM_PUBLISHING.md](./AUTOMATED_NPM_PUBLISHING.md) | **⭐ NEW: Complete automated npm guide** | npm (automated) |
| [NPM_PUBLISHING_GUIDE.md](./NPM_PUBLISHING_GUIDE.md) | Manual npm publishing guide | npm (manual) |
| [GITHUB_PAGES_DEPLOYMENT.md](./GITHUB_PAGES_DEPLOYMENT.md) | Deploy to GitHub Pages | GitHub |
| [GITLAB_PAGES_DEPLOYMENT.md](./GITLAB_PAGES_DEPLOYMENT.md) | Deploy to GitLab Pages | GitLab |
| [Dockerfile](./Dockerfile) | Docker containerization | Docker |

## 🛠️ Development

For developers working on the codebase:

| Document | Description | Topic |
|----------|-------------|-------|
| [ARCHITECTURE.md](./ARCHITECTURE.md) | Technical architecture and design | Architecture |
| [VSCODE_EXTENSION_GUIDE.md](./VSCODE_EXTENSION_GUIDE.md) | Create a VS Code extension | Integration |
| [SECURITY.md](./SECURITY.md) | Security policies and reporting | Security |

## 📋 Reference

Quick reference materials:

| Document | Description | Type |
|----------|-------------|------|
| [CHANGELOG.md](./CHANGELOG.md) | Version history and changes | History |
| [LICENSE](./LICENSE) | MIT license information | Legal |

## 🎯 By Use Case

### "I want to use the CLI"
1. [QUICKSTART_CLI.md](./QUICKSTART_CLI.md) - 60 second start
2. [CLI_USAGE_GUIDE.md](./CLI_USAGE_GUIDE.md) - Complete CLI reference
3. [NPM_PUBLISHING_GUIDE.md](./NPM_PUBLISHING_GUIDE.md) - Publishing to npm

### "I want to create a release"
1. [QUICKSTART_RELEASE.md](./QUICKSTART_RELEASE.md) - 5 minute guide
2. [VERSION_BUMP_QUICKREF.md](./VERSION_BUMP_QUICKREF.md) - Quick version bump reference
3. [.github/RELEASE_GUIDE.md](./.github/RELEASE_GUIDE.md) - Quick reference
4. [scripts/README.md](./scripts/README.md) - Using helper scripts

### "I want to create a pre-release (alpha/beta/RC)"
1. [VERSION_BUMP_QUICKREF.md](./VERSION_BUMP_QUICKREF.md) - **⭐ Quick reference with examples**
2. [PRERELEASE_GUIDE.md](./PRERELEASE_GUIDE.md) - Complete pre-release guide
3. [.github/RELEASE_GUIDE.md](./.github/RELEASE_GUIDE.md) - Pre-release section

### "I want to understand the release automation"
1. [WORKFLOW_ARCHITECTURE.md](./WORKFLOW_ARCHITECTURE.md) - Visual overview
2. [RELEASE_WORKFLOW.md](./RELEASE_WORKFLOW.md) - Detailed documentation
3. [RELEASE_IMPLEMENTATION.md](./RELEASE_IMPLEMENTATION.md) - Implementation details

### "I want to deploy the application"
1. [GITHUB_PAGES_DEPLOYMENT.md](./GITHUB_PAGES_DEPLOYMENT.md) - GitHub Pages
2. [GITLAB_PAGES_DEPLOYMENT.md](./GITLAB_PAGES_DEPLOYMENT.md) - GitLab Pages
3. [Dockerfile](./Dockerfile) - Docker deployment

### "I want to publish to npm"
1. [QUICKSTART_AUTOMATED_RELEASE.md](./QUICKSTART_AUTOMATED_RELEASE.md) - **⭐ 5-minute automated setup**
2. [AUTOMATED_NPM_PUBLISHING.md](./AUTOMATED_NPM_PUBLISHING.md) - Complete automated guide
3. [NPM_PUBLISHING_GUIDE.md](./NPM_PUBLISHING_GUIDE.md) - Manual publishing (fallback)
4. [.npmignore](./.npmignore) - npm package configuration

### "I want to learn how to use the app"
1. [USER_GUIDE.md](./USER_GUIDE.md) - Complete user guide
2. [README.md](./README.md) - Quick start and features
3. [examples/](./examples/) - Example presentations

### "I want to contribute"
1. [README.md](./README.md) - Contributing section
2. [ARCHITECTURE.md](./ARCHITECTURE.md) - Technical architecture
3. [CHANGELOG.md](./CHANGELOG.md) - How to update changelog

### "I want to integrate with VS Code"
1. [VSCODE_EXTENSION_GUIDE.md](./VSCODE_EXTENSION_GUIDE.md) - Complete guide

## 📖 Documentation by Category

### User Documentation
- ✅ How to use the application
- ✅ Features and capabilities
- ✅ Keyboard shortcuts
- ✅ Creating presentations
- ✅ Themes and customization
- ✅ Exporting to PDF

**Read:** [USER_GUIDE.md](./USER_GUIDE.md)

### Release Documentation
- ✅ How to create releases
- ✅ Automated workflows
- ✅ Version management
- ✅ Changelog generation
- ✅ Multi-platform deployment
- ✅ Helper scripts

**Read:** [QUICKSTART_RELEASE.md](./QUICKSTART_RELEASE.md) → [RELEASE_WORKFLOW.md](./RELEASE_WORKFLOW.md)

### Deployment Documentation
- ✅ GitHub Pages setup
- ✅ GitLab Pages setup
- ✅ Docker containerization
- ✅ npm publishing
- ✅ CI/CD pipelines

**Read:** [GITHUB_PAGES_DEPLOYMENT.md](./GITHUB_PAGES_DEPLOYMENT.md)

### Developer Documentation
- ✅ Architecture overview
- ✅ Code structure
- ✅ Component hierarchy
- ✅ State management
- ✅ Build system
- ✅ Testing

**Read:** [ARCHITECTURE.md](./ARCHITECTURE.md)

## 🎓 Learning Path

### Beginner Path (1 hour)
1. [README.md](./README.md) - 5 min - Overview
2. [QUICKSTART_CLI.md](./QUICKSTART_CLI.md) - 2 min - Quick start
3. [USER_GUIDE.md](./USER_GUIDE.md) - 15 min - How to use
4. [QUICKSTART_RELEASE.md](./QUICKSTART_RELEASE.md) - 5 min - First release
5. [examples/](./examples/) - 33 min - Try examples

### Developer Path (2 hours)
1. [README.md](./README.md) - 5 min - Overview
2. [ARCHITECTURE.md](./ARCHITECTURE.md) - 20 min - Architecture
3. [WORKFLOW_ARCHITECTURE.md](./WORKFLOW_ARCHITECTURE.md) - 15 min - Workflow
4. [RELEASE_WORKFLOW.md](./RELEASE_WORKFLOW.md) - 30 min - Release automation
5. [scripts/README.md](./scripts/README.md) - 10 min - Helper scripts
6. Hands-on - 40 min - Create a test release

### DevOps Path (2 hours)
1. [WORKFLOW_ARCHITECTURE.md](./WORKFLOW_ARCHITECTURE.md) - 15 min - Overview
2. [RELEASE_WORKFLOW.md](./RELEASE_WORKFLOW.md) - 30 min - Workflows
3. [RELEASE_IMPLEMENTATION.md](./RELEASE_IMPLEMENTATION.md) - 20 min - Details
4. [GITHUB_PAGES_DEPLOYMENT.md](./GITHUB_PAGES_DEPLOYMENT.md) - 15 min - Pages
5. [NPM_PUBLISHING_GUIDE.md](./NPM_PUBLISHING_GUIDE.md) - 15 min - npm
6. [Dockerfile](./Dockerfile) - 10 min - Docker
7. Hands-on - 15 min - Configure and test

## 🔍 Quick Find

### Configuration Files
- `.github/workflows/*.yml` - GitHub Actions workflows
- `Dockerfile` - Docker configuration
- `.dockerignore` - Docker build exclusions
- `.npmignore` - npm package exclusions
- `package.json` - Project configuration
- `vite.config.ts` - Build configuration
- `tailwind.config.js` - Styling configuration

### Scripts
- `scripts/release.sh` - Release helper script
- `npm run dev` - Development server
- `npm run build` - Production build
- `npm run lint` - Code linting

### Examples
- `examples/tutorial.md` - Tutorial presentation
- `examples/technical-presentation.md` - Technical template
- `examples/product-demo.md` - Product demo template

## 📞 Getting Help

### For Users
- **Question:** Read [USER_GUIDE.md](./USER_GUIDE.md)
- **Bug:** Check [SECURITY.md](./SECURITY.md) then open an issue
- **Feature:** Open an issue with enhancement label

### For Developers
- **Setup:** Read [README.md](./README.md) development section
- **Architecture:** Read [ARCHITECTURE.md](./ARCHITECTURE.md)
- **Release:** Read [RELEASE_WORKFLOW.md](./RELEASE_WORKFLOW.md)
- **Contributing:** Read [README.md](./README.md) contributing section

### For DevOps
- **CI/CD:** Read [WORKFLOW_ARCHITECTURE.md](./WORKFLOW_ARCHITECTURE.md)
- **Deployment:** Read deployment guides
- **Automation:** Read [RELEASE_IMPLEMENTATION.md](./RELEASE_IMPLEMENTATION.md)

## 🎯 Documentation Quality

All documentation includes:
- ✅ Clear objectives
- ✅ Step-by-step instructions
- ✅ Code examples
- ✅ Troubleshooting sections
- ✅ Visual aids (where applicable)
- ✅ Links to related docs

## 📝 Contributing to Documentation

Found an issue or want to improve docs?

1. **Small fixes:** Edit directly and open PR
2. **Large changes:** Open an issue first to discuss
3. **New docs:** Follow existing format and style
4. **Updates:** Keep CHANGELOG.md in sync

## 🗺️ Documentation Roadmap

Future documentation planned:
- [ ] Video tutorials
- [ ] API reference
- [ ] Plugin development guide
- [ ] Performance optimization guide
- [ ] Internationalization guide
- [ ] Accessibility guide

---

**Quick Links:**

| Need | Document |
|------|----------|
| 🚀 Get started | [README.md](./README.md) |
| 📦 Create release | [QUICKSTART_RELEASE.md](./QUICKSTART_RELEASE.md) |
| 📖 Learn to use | [USER_GUIDE.md](./USER_GUIDE.md) |
| 🏗️ Architecture | [ARCHITECTURE.md](./ARCHITECTURE.md) |
| 🔄 Workflows | [WORKFLOW_ARCHITECTURE.md](./WORKFLOW_ARCHITECTURE.md) |
| 🌐 Deploy | [GITHUB_PAGES_DEPLOYMENT.md](./GITHUB_PAGES_DEPLOYMENT.md) |

---

**Happy reading! 📚**
